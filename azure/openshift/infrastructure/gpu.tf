...
resource "kubernetes_namespace" "gpu" {
  ...
  metadata {
    ...
    name = var.nvidia_operator_namespace
  }
}
...
resource "kubectl_manifest" "gpu" {
  ...
  yaml_body = <<YAML
    ...
    metadata:
      ...
      namespace: ${var.nvidia_operator_namespace}
    spec:
...
resource "null_resource" "wait-for-gpu-operator" {
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "${path.module}/wait-for-subscription.sh ${var.nvidia_operator_namespace} gpu-operator-certified"
  }
}
