...
locals {
  ...
  environment_all_subnets_sg_id                      = var.load_environment == "" ? (var.network_module == "networking" ? module.networking[0].all_subnets_sg_id : module.security-group.all_subnets_sg_id) : data.terraform_remote_state.environment[0].outputs.all_subnets_sg_id
  ...
  environment_nginx_ingress_security_group_id        = var.load_environment == "" ? (var.create_nginx_ingress_security_group && var.network_module == "networking" && var.network_type == "create" ? module.networking[0].nginx_ingress_security_group_id : "null") : data.terraform_remote_state.environment[0].outputs.nginx_ingress_security_group_id
  environment_nat_gateway_eips                       = var.load_environment == "" ? (var.network_module == "networking" && var.network_type == "create" ? module.networking[0].nat_gateway_eips : ["null"]) : data.terraform_remote_state.environment[0].outputs.nat_gateway_eips
